...
Run `jarvis-registry skills sync --mode copilot $path`, substituting `$path` verbatim (not the
literal text `$path`).
...
to run `jarvis-registry skills sync --mode copilot $path` themselves from a real terminal once, to
