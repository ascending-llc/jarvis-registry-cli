---
name: sync-skills
description: Sync the latest Jarvis Registry skills into GitHub Copilot from the user's Jarvis Registry account.
---
...
If the user specified a project directory, run `jarvis-registry skills sync --mode copilot $path`,
substituting `$path` verbatim (not the literal text `$path`). Otherwise, run
`jarvis-registry skills sync --mode copilot` with no path argument at all. Never pass
...
to run `jarvis-registry skills sync --mode copilot` themselves from a real terminal once (carrying
through `$path`, if the user specified one), to confirm it's safe for the CLI to manage that folder.

When no project directory was given, the sync summary table has a `Link` column reporting whether
each skill was made visible to Copilot under `~/.copilot/skills/`. Most rows read `Linked`,
`Unchanged`, or `Relinked` and need no action. A row reading `Skipped` means something else already
exists at that name under `~/.copilot/skills/` and was left untouched — tell the user their options:
remove or move the conflicting file/folder themselves, set `local.skills.link.override: true` in
